-- +goose Up
-- Add C-025 safe Document MCP query/read tools to the untouched system default.
-- User-created or explicitly customized QA configs remain authoritative.
-- Material metadata tools stay opt-in until their permission model is confirmed.
UPDATE qa_config_versions
SET enabled_tool_names = enabled_tool_names || '["document__list_reports", "document__get_report", "document__list_report_files", "document__read_report_file"]'::jsonb
WHERE version_no = 1
